...
\begin{filebox}{src/data/setup.py}
  ...
  RGB and CHM channels, and saves everything to disk as \texttt{.pt} files for
...
\begin{enumerate}
  ...
  \item Saves the \texttt{(image\_tensor, bounding\_boxes)} pair to a
        \texttt{.pt} file.
\end{enumerate}
...
the canvas size (the image dimensions). Torchvision's transform pipeline uses
this metadata to update bounding box coordinates correctly when the image is
cropped or flipped during augmentation.
...
Loading from \texttt{.pt} files means \texttt{\_\_getitem\_\_} is fast: a
single \texttt{torch.load} call, then the transform pipeline. No image
decoding, no XML parsing, no resizing -- all of that was done once during
setup.
...
\begin{enumerate}[nosep]
  \item \textbf{RandomCrop(400)} -- cuts a random $400 \times 400$ pixel
        window from the image. This is data augmentation: by showing the model
        different crops of the same tile each time, we effectively multiply the
        size of the training set without needing more labelled data.
  ...
  \item \textbf{SanitizeBoundingBoxes} -- removes any bounding boxes that
        ended up fully outside the cropped region after \texttt{RandomCrop},
        and clamps any partially-outside boxes to the image edge. Without this
        step, the model would receive annotations pointing to regions that do
        not exist in the cropped image.
\end{enumerate}

Note that \texttt{SanitizeBoundingBoxes} only works correctly because the
bounding boxes are stored as a Torchvision \texttt{BoundingBoxes} object
(rather than a plain tensor). Torchvision's transforms know to treat
\texttt{BoundingBoxes} specially, applying the same geometric transformation
to the boxes as to the image. If the boxes were stored as a plain
\texttt{torch.Tensor}, the \texttt{RandomCrop} would move the pixels but
leave the box coordinates unchanged -- a silent, hard-to-detect bug.
